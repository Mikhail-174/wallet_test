...
class AsyncWalletView(APIView):
    serializer_class = AsyncWalletSerializer                                        #async

    async def get_object(self, id):                                                 #async
        try:
            wallet = await Wallet.objects.aget(id=id)                #async
            return wallet
        except Wallet.DoesNotExist:
            return None
    ...
    async def post(self, request, *args, **kwargs):         #async
        operation = request.data.get('operation_type')       #async
        amount = decimal.Decimal(request.data.get('amount'))  #async

        from django.db import transaction
        wallets = Wallet.objects.select_for_update().filter(id=kwargs['id'])
